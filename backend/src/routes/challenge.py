...
# The following is scheme that is something that fastapi can use to validate
# that the data being sent to the endpoint is correct
...
async def generate_challenge(request: ChallengeRequest,request_obj:Request, db: Session = Depends(get_db)):
    try:
        ...
        if quota.quota_remaining <= 0:
            raise HTTPException(status_code=429, detail="Daily Quota Limit Reached")
